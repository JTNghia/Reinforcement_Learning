...
class CustomEnv:
    ...
    def __init__(self, df, initial_balance=1000, lookback_window_size=50):
        ...
        self.state_size = (self.lookback_window_size, 14)
        self._profits = [] # for visualization
        self._history_profits = [] # for visualization
    ...
    def step(self, action):
        ...
        self.crypto_sold = 0
        self.current_step += 1
        if self.net_worth <= self.initial_balance/2 or self.current_step >= len(self.df):
            done = True  
            return {}, {}, done
        else:
            ...
            self._profits.append(reward) # for visualization
            self._history_profits.append(self.df.loc[self.current_step, 'Date']) # for visualization
    ...
    def render_all(self, namefile):
        plt.figure(figsize=(100,60))
        plt.plot(self._profits)
        plt.title(f'Average  profit: {np.mean(self._profits)}')
        plt.xlabel('Day')
        plt.ylabel('Profit')
