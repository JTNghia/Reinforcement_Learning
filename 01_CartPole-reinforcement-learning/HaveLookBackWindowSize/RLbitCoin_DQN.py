...
class DQNAgent:
    def __init__(self):
        ...
        self.state_size = 10*lookback_window_size
        ...
        for e in range(self.EPISODES):
            ...
            while not done:
                ...
                if done:                   
                    ...
                    self.env.render_all(f"SaveModel/DQN_HVN_Episode({e})")
                    self.env._profits = []
                    self.env._networths = []
                    break
                ...
                self.replay()
    def retrain(self):
        self.load("SaveModel_11_7(2)/DQN_HVN_Episode(312).h5")
        for e in range(313, self.EPISODES):
            state = self.env.reset()
            # print(f'state {state}, shape {state.shape}\n\n\n\n\n\n\n\n\n\n\n')
            state = np.reshape(state, [1, self.state_size])
            done = False
    ...
    def test(self):
        self.load("SaveModel_11_7/DQN_HVN_Episode(85).h5")
        for e in range(self.EPISODES):
            state = self.env.reset()
            state = np.reshape(state, [1, self.state_size])
            done = False
